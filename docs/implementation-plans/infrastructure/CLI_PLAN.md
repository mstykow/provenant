...
| `--package-only`                                     | Restricts output to package-focused results                  | `Done`     | Conflicts with ScanCode-incompatible mixes such as `--license`/`--summary`; skips top-level package assembly and trims output to package-bearing files and ancestor directories.                                                |
