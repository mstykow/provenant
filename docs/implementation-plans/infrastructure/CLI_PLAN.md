...
| `--package-in-compiled`                              | Extracts package metadata from compiled artifacts            | `Done`     | Adds compiled-binary package extraction for current Go binaries (`go version -m`) and Rust binaries with cargo-auditable `.dep-v0` sections, while regular archive package handlers remain part of normal `--package` scanning.                                                        |
| `--package-only`                                     | Restricts output to package-focused results                  | `Done`     | Conflicts with ScanCode-incompatible mixes such as `--license`/`--summary`; skips top-level package assembly and trims output to package-bearing files and ancestor directories. This does **not** implicitly enable `--package-in-compiled`; compiled binary scanning remains opt-in. |
