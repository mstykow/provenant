...
#[test]
fn mark_generated_files_accepts_absolute_paths_without_scan_root() {
    let temp = tempfile::tempdir().expect("tempdir should be created");
    let generated_path = temp.path().join("generated_6.c");
    std::fs::write(
        &generated_path,
        "/* DO NOT EDIT THIS FILE - it is machine generated */\n",
    )
    ...
    let mut files = vec![file(
        generated_path
            .to_str()
            .expect("absolute temp path should be UTF-8"),
    )];

    mark_generated_files(&mut files, None);

    assert_eq!(files[0].is_generated, Some(true));
}

#[test]
fn mark_generated_files_skips_unanchored_relative_paths_without_scan_root() {
    let mut files = vec![file(
        "testdata/summarycode-golden/generated/simple/generated_6.c",
    )];
...
#[test]
fn mark_generated_files_preserves_existing_generated_flags() {
    let mut generated = file("virtual/generated.c");
    generated.is_generated = Some(true);

    let mut plain = file("virtual/plain.c");
    plain.is_generated = Some(false);

    let mut files = vec![generated, plain];

    mark_generated_files(&mut files, None);

    assert_eq!(files[0].is_generated, Some(true));
    assert_eq!(files[1].is_generated, Some(false));
}
